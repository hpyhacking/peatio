$(function(){
  ...
  var sels = $('select#deposits_bank_fund_extra').selectize();
  $('select#deposits_bank_fund_uid').selectize({
    ...
    load: function(query, callback) {
      ...
      if(query.length < 4){
        $.ajax({
          url: '/fund_sources?channel_id=' + $('input#deposits_bank_channel_id').val() + '&query=' + encodeURIComponent(query),
          type: 'GET',
          error: function() {
            callback();
          },
    ...
    onItemAdd: function(value, $item) {
      var extra = $item.text();
      window.item = $item;
      console.log(value, extra, $item);
      $item.text(value);
      $('form input#deposits_bank_fund_extra').val(extra);

      if(sels[0]){
        var index = extra.indexOf(' ');
        if(index >= 0){
          $('form input#deposits_bank_subbranch').val(extra.slice(index+1));
