module Worker
  class FundStats < Stats
    ...
    def point_1(from)
      to = from + 1.minute
      deposits = Deposit.with_aasm_state(:accepted).where(currency: @currency, created_at: from...to).pluck(:amount)
      withdraws = Withdraw.with_aasm_state(:done).where(currency: @currency, created_at: from...to).pluck(:amount)
