  ...
  describe '#create_address!' do
    ...
    context 'when address is not generated immediately' do
      ...
      it 'raises an error' do
        expect { subject }.to raise_error(CoinAPI::Error) { |e| expect(e.message).to match /bitgo hasn't returned an address/i }
      end

      context 'when non-bang method is being used' do
        subject { client.create_address }
